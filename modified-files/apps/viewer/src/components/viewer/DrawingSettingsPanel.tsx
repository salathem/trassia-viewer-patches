...
export function DrawingSettingsPanel({ onClose }: DrawingSettingsPanelProps) {
  ...
  // Trassia (UPSTREAM-RESTE, TODO #47, Tester M-4 04.09.): dieses Fenster ist
  // ein eigener schwebender Kasten ohne Escape-Weg — Escape lief durch bis zum
  // globalen Kuerzel, schaltete das Schnittwerkzeug ab, und das Fenster blieb
  // stehen. Wie der Anpassen-Dialog der Leiste (CustomizeSidebar): Fokus beim
  // Oeffnen hinein (einmal), Escape schliesst NUR dieses Fenster und markiert
  // die Taste per `preventDefault` als verbraucht (Riegel in
  // useKeyboardShortcuts). Ein offenes Radix-Aufklappfeld (Select) im Fenster
  // behaelt sein Escape: Radix schliesst zuerst und setzt `defaultPrevented`.
  const wurzelRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    wurzelRef.current?.focus();
  }, []);
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key !== 'Escape' || e.defaultPrevented) return;
      const ziel = e.target as HTMLElement | null;
      if (ziel?.closest?.('[data-radix-popper-content-wrapper], [role="listbox"], [role="menu"]')) return;
      e.preventDefault();
      onClose();
    };
  ...
  return (
    <div
      ...
      tabIndex={-1}
      className="flex flex-col h-full bg-background border-l outline-none"
    >
      {/* Header */}
