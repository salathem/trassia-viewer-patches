 ...
 *
 * - `free` panels use stored window coordinates, keeping their header
 *   below the toolbar when bounds are available.
...
export function computeFloatingPanelStyle(p: FloatingPanelState, bounds: SnapBounds | null): CSSProperties {
  if (p.snap === 'free') return { left: p.x, top: bounds ? Math.max(p.y, bounds.top) : p.y, width: p.w, height: p.h };
