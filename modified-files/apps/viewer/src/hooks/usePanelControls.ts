...
export function usePanelControls(): PanelControls {
  ...
  const closePanel = useCallback((id: WorkspacePanelId) => {
    ...
    // Trassia (Paket SEITENLEISTE, Tester-Blocker B-1): ein Seitenpanel OHNE
    // eigenes Sichtbarkeits-Flag (Zones, Trassia-Panels drape/kubatur/
    // laengsschnitt/entwurf) wird nur ueber `sidebarActivePanel` gefuehrt —
    // fuer die ist `setDockedVisible` ein No-op und das X in der Kopfzeile
    // schloss nichts. Ist das Panel danach immer noch das aktive, schliesst
    // dieselbe Aktion wie der zweite Klick auf sein Symbol. Flag-Panels sind
    // hier nie mehr aktiv (die Exklusivitaets-Subscription hat synchron
    // umgeschaltet), darum greift die Zeile nur im flaglosen Fall.
    if (useViewerStore.getState().sidebarActivePanel === id) {
